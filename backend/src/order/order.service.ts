...
export class OrderService {
  ...
  async createPayment(dto: OrderDto, userId: string) {
    ...
    const result = await this.prisma.$transaction(async (prisma) => {
      const order = await prisma.order.create({
        data: {
          status: dto.status as unknown as OrderStatus,
          total,
          user: {
            connect: {
              id: userId,
      ...
      const payment = await checkout.createPayment({
        ...
        description: `Оплата заказа в магазине Internet Shop. ID платежа: #${order.id}`,
        metadata: {
          orderId: order.id,
        },
      });
  ...
  async updateOrderStatus(dto: PaymentStatusDto) {
